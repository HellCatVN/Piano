$(function(){
	...
	*/
	...
	$(".piano_function").on("click", ".start_record", function(){
		...
		$(".cancel_record").show();
		$(".finish_record").show();
		$(".record-state").text("Đang ghi âm, vui lòng sử dụng chuột hoặc bàn phím để chơi");
		$(this).hide();
	});
	...
	$(".piano_function").on("click", ".upload_record", function(){
		...
		if($(".share_record").data("copy")){
			...
			savePopShow("Gửi thất bại", position, "error");
			return false;
		}
	});
	...
	*/
	var list = {
		"Little Star ✩":"11556650443322105544332055443320115566504433221",
		"Jasmine":"33568865056503356886505650555356066503235032101210321203568650523532121",
		"Cậu chuyện":"555653323138868650886865565135565320555653323138868650886865565358886789",
		"Find a Friend":"5656565058765530553355302432121",
		"Schools":"12315066865066805630653531231",
		"Penny":"5868503523503568565351032032123065356058865630523210",
		"Chinh phục":"58755650587563606665334044456322",
		"Mua Đông Lạnh":"4440456880689665606542206542202455555658",
		"Vị Biển":"998654065421012124456809986545",
		"Chuyện cổ tihcs":"543034303434321013560665224301356066765434321",
		"Bắc Kinh ❤":"58543210112331345058543520432650231536876750999870675053687678905678905888",
		"Limited Love":"5555345706666465055557654044443210888856888809999876077777890507789878",
		"Hồ Lô Biến ✪":"1130113066656513086656051207535",
		"Ước Muốn":"3230321556503566666350366656685556312223212032303215565035666663503666566855531012223211018888986563012012320188898656805605689",
		"Big Big World":"1233303422202311101232102333402222301322",
		"Happy BirthDay":"556587055659805538760443124",
		"Kỷ niệm":"111543212330135666876545566687655321023434211023434311|羽毛公主|http:\/\/bbs.360safe.com\/forum.php?mod=redirect&goto=findpost&ptid=1969236&pid=13529820",
		"Gấu & Búp Bê":"1023455543444321351023455543444321316066545554344432135606654555434044321031|wangsongha1986lzq|http:\/\/bbs.360safe.com\/forum.php?mod=redirect&goto=findpost&ptid=1969236&pid=13527397"
	...
	function renderList(list){
		...
		tmp += '<li class="mysong"><span class="split">•</span><a href="https://phongvan.gotank.me/piano/" data-title="'+i+'">Nghe thêm »<a></li>';
		tmp += "</ul>";
	...
	function renderSong(title){
		...
		if(keys.length){					//全部遍历
			...
			for(var i=0;i<len;i++){
				...
				$("#opern-info")[0].innerHTML += '<dl class="number"><dt>Số</dt>'+tmp1+'</dl>';
				$("#opern-info")[0].innerHTML += '<dl class="letter"><dt>Chữ</dt>'+tmp2+'</dl>';
			}
		}
		// 是否有曲谱贡献者
		if(arr[1]){
