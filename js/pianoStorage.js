    ...
    */
	function insertData(){
		...
		if(dataKey == null || dataKey ==""){
			...
			savePopShow("Lưu thành công (Vào Menu mục Đã Lưu để nghe lại)", position);
		}else{
			savePopShow("Tên trùng khớp, vui lòng đặt tên khác", position, "error");
		}
	}
    ...
    */
	...
	$(".my_album").on("click", function(){
		...
		if(!empty){
			// 布局曲谱
			renderList(saveList);
			$(".song-list h3").text("Các bài hát đã lưu");
		}else{
			// 已全部被删除，重置存储id
			$("#list-info").empty();
			LocalStorage.setItem("musicKey","0");
			$(".song-list h3").text("Chưa có bài hát được được lưu");
		}
